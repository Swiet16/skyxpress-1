...
function drawHeaderBand(doc: jsPDF, entry: ManifestEntry, countryMap: Record<string, string>): number {
  ...
  // Typed as objects (not tuples) so strict TS mode can't widen elements
  // to `string | undefined` and fail the build.
  type InfoChip = { label: string; value: string };
  const infoChips: InfoChip[] = [
    { label: "Date",       value: entry.manifestDate ? String(entry.manifestDate) : "—" },
    { label: "Flight No",  value: String(entry.flightNo || "—") },
    { label: "Run No",     value: String(entry.runNumber || "—") },
    { label: "Service",     value: String(entry.service || entry.serviceType || "—") },
    { label: "Forwarder",  value: String(entry.forwarder || "—") },
    { label: "Origin Hub", value: String(entry.originHub || "—") },
    { label: "Dest Hub",   value: String(entry.destinationHub || "—") },
    { label: "Bags",       value: String(entry.noOfBags ?? "—") },
    { label: "Total AWBs", value: String(entry.parcels?.length || 0) },
  ];
  ...
  infoChips.forEach((chip, i) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(6.5);
    doc.text(chip.label.toUpperCase(), x + 3, cy + 3.5);
